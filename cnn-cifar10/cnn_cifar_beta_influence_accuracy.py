...
if __name__ == '__main__':
	...
	for number in range(1, 11, 1):
		beta = number / 10
		alpha = 1.0
		stack_x1_again = x_svd + alpha * beta * stack_x1_again
		...
		for epoch in range(EPOCH):
			...
			print('Train Round {:3d}, Train Average Loss {:.3f}, Train acc {}'.
			      format(epoch + 1, train_loss, train_accuracy * 100))
			...
			print('Test Round {:3d}, Test Average Loss {:.3f}, Test acc {}'.
			      format(epoch + 1, test_loss, test_accuracy * 100))
		...
		wandb.log({"Train acc": train_accuracy * 100, 'beta': beta})
		wandb.log({"Test acc": test_accuracy * 100, 'beta': beta})