...
if __name__ == '__main__':
	...
	updated_weight = updated_weight_fc2[:16, :75].reshape(16, 3, 5, 5)
	updated_weight_1 = torch.from_numpy(updated_weight)
	# 更新模型的第二层权重矩阵
	w_global['conv1.0.weight'] = updated_weight_1
	...
	for epoch in tqdm(range(EPOCH)):
		...
		# 将结果保存到 Excel文件
		with pd.ExcelWriter('100_cnn_cifar_ISVD_fedavg_train_results.xlsx') as writer_1:
			results_df.to_excel(writer_1, index=False, sheet_name='Results')
		
		with pd.ExcelWriter('100_cnn_cifar_ISVD_fedavg_test_results.xlsx') as writer_2:
