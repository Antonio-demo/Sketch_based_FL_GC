...
updated_weight = updated_weight_fc2[:, :9].reshape(32,1,3,3)

# 更新模型的第二层权重矩阵
updated_weight_1 = torch.from_numpy(updated_weight)

# 更新模型的第二层权重矩阵
w_global['features.0.weight'] = updated_weight_1
...
for epoch in tqdm(range(num_epochs)):
	...
	# 将结果保存到 Excel文件
	with pd.ExcelWriter('100_alexnet_fmnist_ISVD_fedavg_train_results.xlsx') as writer_1:
		results_df.to_excel(writer_1, index=False, sheet_name='Results')
	
	with pd.ExcelWriter('100_alexnet_fmnist_ISVD_fedavg_test_results.xlsx') as writer_2:
