...
updated_weight = updated_weight_fc2[:, :48].reshape(64,3,4,4)
# 更新模型的第二层权重矩阵
updated_weight_1 = torch.from_numpy(updated_weight)
w_global['conv1.conv.weight'] = updated_weight_1
...
for epoch in tqdm(range(num_epochs)):
	...
	# 将结果保存到 Excel文件
	with pd.ExcelWriter('100_googlenet_cifar_ISVD_fedavg_train_results.xlsx') as writer_1:
		results_df.to_excel(writer_1, index=False, sheet_name='Results')
	
	with pd.ExcelWriter('100_googlenet_cifar_ISVD_fedavg_test_results.xlsx') as writer_2:
