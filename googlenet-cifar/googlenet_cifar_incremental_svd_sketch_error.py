...
tensor_x = torch.from_numpy(array_countsketch).reshape(64,3,4,4)

error_accumulate = x - countsketch_x_1 #40*48

error_accumulate = torch.cat((error_accumulate, b_1), 0).reshape(64,3,4,4)#64*48
...
for epoch in tqdm(range(num_epochs)):
	...
	# 将结果保存到 Excel文件
	with pd.ExcelWriter('100_googlenet_cifar_ISVD_sketch_error_fedavg_train_results.xlsx') as writer_1:
		results_df.to_excel(writer_1, index=False, sheet_name='Results')
	
	with pd.ExcelWriter('100_googlenet_cifar_ISVD_sketch_error_fedavg_test_results.xlsx') as writer_2:
