	...
	def train(self, repeat):
		...
		if self.optim == 'adam':
			trainer = mx.gluon.Trainer(params, self.optim,
			                           {'learning_rate': self.lr, 'wd': self.wd, 'beta1': self.beta1,
			                            'beta2': self.beta2, 'epsilon': self.epsilon, 'lr_scheduler': self.scheduler},
			                           kvstore='device')
		else:
			trainer = mx.gluon.Trainer(params, self.optim,
			                           {'learning_rate': self.lr, 'wd': self.wd, 'momentum': self.momentum,
		...
		for epoch in range(self.epochs):
			...
			f = open('./results/grid-{}-lr-{}-wd-{}-mom-{}-rep-{}'.format(self.optim, self.lr, self.momentum, self.wd,
			                                                              repeat), 'wb')
