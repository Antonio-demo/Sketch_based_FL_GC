...
class UserEncoder(torch.nn.Module):
	def __init__(self, args, word_embedding, uet_encoder=None, uet_reduce_linear=None,
		...
		if args.use_padded_news_embedding:
			...
			self.pad_doc = nn.Parameter(torch.empty(1, args.num_attention_heads * 20).uniform_(-1, 1)).type(
				torch.FloatTensor)
		else:
	...
	def _process_news(self, vec, mask, pad_doc,
		...
		if use_padded_embedding:
			...
			# batch_size, maxlen, dim
			vec = vec * mask.unsqueeze(2).expand(-1, -1, self.args.num_attention_heads * 20) + padding_doc * (
						1 - mask.unsqueeze(2).expand(-1, -1, self.args.num_attention_heads * 20))
		# batch_size, news_dim
