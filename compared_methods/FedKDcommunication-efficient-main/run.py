...
def train(args):
	...
	config_student = config_class.from_pretrained(args.config_name if args.config_name else args.model_name_or_path,
	                                              output_hidden_states=True)
	...
	for i in range(client_num):
		bert_model_stu = model_class.from_pretrained(args.model_name_or_path,
		                                             from_tf=bool('.ckpt' in args.model_name_or_path),
		                                             config=config_student)
		bert_model_tea = model_class.from_pretrained(args.model_name_or_path,
	...
	local_models = []
	for i in range(client_num):
		local_client = ModelBert(args, teacher_bert_models[i], student_bert_models[i], len(category_dict),
	...
	latest_local_student_para = [
		{name: para.detach().cpu().numpy() for name, para in model.student_bert_models.named_parameters()} for model in
		local_models]
	...
	for comm_round in range(args.epochs):
		for i in range(client_num):
			...
			for cnt, (log_ids, log_mask, input_ids, targets,
			          uet_ids, uet_mask, bing_ids, bing_mask, user_ids, random_news, catetarget) in enumerate(
				dataloader):
				...
				bz_loss, y_hat = local_models[i](input_ids, log_ids, log_mask, targets, uet_ids, uet_mask, bing_ids,
				                                 bing_mask, user_ids, random_news, catetarget)
		...
		for i in range(client_num):
			...
			for name in latest_local_student_para[i]:
				student_update[name] = local_models[i].student_bert_models.state_dict()[name].detach().cpu().numpy() - \
				                       latest_local_student_para[i][name]
			...
			for i in range(client_num):
				if len(all_compressed_update[i][name]) == 3:
					...
					aggregated_para[name].append(
						np.dot(np.dot(all_compressed_update[i][name][0], all_compressed_update[i][name][1]),
		...
		for name in aggregated_para:
			if len(aggregated_para[name]) == 3:
				aggregated_para[name] = np.dot(np.dot(aggregated_para[name][0], aggregated_para[name][1]),
				                               aggregated_para[name][2])
		for i in range(client_num):
			for name in aggregated_para:
				for name2 in local_models[i].student_bert_models.state_dict():
					if name in name2:
						local_models[i].student_bert_models.state_dict()[name][:] = latest_local_student_para[i][
							                                                            name] + torch.FloatTensor(
							aggregated_para[name]).cuda()
		latest_local_student_para = [
			{name: para.detach().cpu().numpy() for name, para in model.student_bert_models.named_parameters()} for model
			in local_models]
...
def test(args):
	...
	if args.apply_turing:
		...
		config_class, model_class, tokenizer_class = MODEL_CLASSES[args.model_type]
		config2 = config_class.from_pretrained(args.config_name if args.config_name else args.model_name_or_path,
		                                       output_hidden_states=True)
	...
	for cnt, (log_vecs, log_mask, news_vecs, news_bias, labels, \
		...
		if args.process_uet or args.process_bing:
			...
			user_vecs = model.user_encoder(log_vecs, log_mask, uet_ids, \
			                               uet_mask, bing_ids, bing_mask, user_ids).to(
		...
		if cnt % args.log_steps == 0:
			for i in range(2):
				print_metrics(hvd_rank, cnt * args.batch_size, get_mean([AUC[i], MRR[i], nDCG5[i], \
				                                                         ...
				                                                         CTR10[i]]))
				if args.debias:
					print_metrics(hvd_local_rank, cnt * args.batch_size,
					              get_mean([AUC_add_bias[i], MRR_add_bias[i], nDCG5_add_bias[i], \
					                        nDCG10_add_bias[i], CTR1_add_bias[i], CTR3_add_bias[i], CTR5_add_bias[i],
					                        CTR10_add_bias[i]]))
				if args.process_uet or args.process_bing:
					print_metrics(hvd_local_rank, cnt * args.batch_size, get_mean([AUC_on[i], MRR_on[i], nDCG5_on[i], \
					                                                               nDCG10_on[i], CTR1_on[i], CTR3_on[i],
					                                                               CTR5_on[i], CTR10_on[i]]))
