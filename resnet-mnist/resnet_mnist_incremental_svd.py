...
updated_weight = updated_weight_fc2[:, :49].reshape(64,1,7,7)

updated_weight_1 = torch.from_numpy(updated_weight)
w_global['conv1.weight'] = updated_weight_1
...
for epoch in tqdm(range(NUM_EPOCHS)):
	...
	# 将结果保存到 Excel文件
	with pd.ExcelWriter('100_resnet_mnist_ISVD_fedavg_train_results.xlsx') as writer_1:
		results_df.to_excel(writer_1, index=False, sheet_name='Results')
	
	with pd.ExcelWriter('100_resnet_mnist_ISVD_fedavg_test_results.xlsx') as writer_2:
