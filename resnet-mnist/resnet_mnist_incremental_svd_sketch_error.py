...
alpha= 0.1
countsketch_x = tensor_x_1 + alpha * error_accumulate
#print(countsketch_x.shape)#16*49

#将countsketch_x重新转换成原来的shape
expand_x = torch.cat((countsketch_x, b_2), dim=0).reshape(64, 1, 7, 7)
#print(st.shape)

w_global['conv1.weight'] = expand_x
...
for epoch in tqdm(range(NUM_EPOCHS)):
	...
	# 将结果保存到 Excel文件
	with pd.ExcelWriter('100_resnet_mnist_ISVD_sketch_error_fedavg_train_results.xlsx') as writer_1:
		results_df.to_excel(writer_1, index=False, sheet_name='Results')
	
	with pd.ExcelWriter('100_resnet_mnist_ISVD_sketch_error_fedavg_test_results.xlsx') as writer_2:
