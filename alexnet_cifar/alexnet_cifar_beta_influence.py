...
f1 = open('/home/user/tmp/GoogleNet_CIFAR10/alexnet_cifar/beta_influence/alex_cifar_beta_error_train_loss_acc.csv', 'w')
csv_writer1 = csv.writer(f1)

line1_1 = ['beta', 'epoch', 'train loss', 'train acc', 'train time']
csv_writer1.writerow(line1_1)


f2 = open('/home/user/tmp/GoogleNet_CIFAR10/alexnet_cifar/beta_influence/alex_cifar_beta_error_test_loss_acc.csv', 'w')
csv_writer2 = csv.writer(f2)

line2_1 = ['beta', 'epoch', 'test loss', 'test acc', 'test time']
...
for number in range(1, 11, 1):
	...
	tensor_x_2 = tensor_x_1 + alpha * error_accumulate + beta * alpha * error_accumulate
	...
	for epoch in range(num_epochs):  # loop over the dataset multiple times
		...
		line1 = [beta, epoch + 1, round(float(loss.item()), 4), train_acc, round(float(train_time), 4)]
		...
		line2 = [beta, epoch + 1, round(float(loss_1.item()), 4), test_acc, round(float(elapsed_2), 4)]

		csv_writer2.writerow(line2)

	wandb.log({"Train acc": train_acc, 'beta': beta})
	wandb.log({"Test acc": test_acc, 'beta': beta})
...
# save model
torch.save(model.state_dict(), './alexnet_cifar_beta_influence.pt')
