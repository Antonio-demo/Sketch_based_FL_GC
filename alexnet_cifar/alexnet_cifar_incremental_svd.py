...
updated_weight = updated_weight_fc2[:, :27].reshape(32,3,3,3)

# 更新模型的第二层权重矩阵
updated_weight_1 = torch.from_numpy(updated_weight)

w_global['layer1.0.weight'] = updated_weight_1
...
for epoch in tqdm(range(num_epochs)):  # loop over the dataset multiple times
	...
	# 将结果保存到 Excel文件
	with pd.ExcelWriter('100_alexnet_cifar_ISVD_fedavg_train_results.xlsx') as writer_1:
		results_df.to_excel(writer_1, index=False, sheet_name='Results')
	
	with pd.ExcelWriter('100_alexnet_cifar_ISVD_fedavg_test_results.xlsx') as writer_2:
