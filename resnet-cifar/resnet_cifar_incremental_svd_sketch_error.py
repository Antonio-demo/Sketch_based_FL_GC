...
# 更新模型的第二层权重矩阵
updated_weight = updated_weight_fc2[:, :27]#numpy
...
countsketch_x = tensor_x + alpha * error_accumulate

expand_x = torch.cat((countsketch_x, b_2), 0).reshape(64,3,3,3)

w_global['conv1.0.weight'] = expand_x
...
for epoch in tqdm(range(NUM_EPOCHS)):
	...
	# 将结果保存到 Excel文件
	with pd.ExcelWriter('100_resnet_cifar_ISVD_sketch_error_fedavg_train_results.xlsx') as writer_1:
		results_df.to_excel(writer_1, index=False, sheet_name='Results')

	with pd.ExcelWriter('100_resnet_cifar_ISVD_sketch_error_fedavg_test_results.xlsx') as writer_2:
